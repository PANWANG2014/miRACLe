...
#sum_miracle is used to calculate the denominator of the miracle method
...
#miracle_score is used to get the miracle score
miracle_score = function(mirna_use1, mrna_use1, CWCS, sumup){
  mirna_use = matrix(as.numeric(mirna_use1), nrow = nrow(mirna_use1))
  mrna_use = matrix(as.numeric(mrna_use1), nrow = nrow(mrna_use1))
  pos = which(CWCS != 0)
  miracle_matrix = matrix(data = 0, ncol = ncol(mrna_use), nrow = length(pos), byrow = TRUE)
  miracle_rank = matrix(data = 0, ncol = ncol(mrna_use), nrow = length(pos), byrow = TRUE)
  
  for(m in 1:ncol(mrna_use)){
    pro_temp = (mirna_use[,m] %*% t(mrna_use[,m])) * CWCS/sumup[m]
    rank_temp = rank(-pro_temp, ties.method = "min")
    miracle_matrix[, m] = pro_temp[pos]
    miracle_rank[, m] = rank_temp[pos]
  }
  return(list(miracle_matrix, pos))
}
...
#feature_select_miracle is used to get the features
feature_select_miracle = function(mirna_E, mirna_T, temp){
...
#feature_trans is used to transfer the miracle-predicted pairs into their mirna name and mrna name
...
#sequence information: CWCS_matrix, mirna list, mrna list
...
CWCS_matrix_cwcs1 = as.matrix(read.table("CWCS_matrix.txt", head = TRUE, sep = "\t"))
CWCS_matrix_cwcs = abs(CWCS_matrix_cwcs1)
...
#the following code is used to get the result of miracle score, which is z6_E and z6_T
z4_E = mirna_mrna_loc(z3_E[[3]], z3_E[[4]], mirna, mrna, CWCS_matrix_cwcs)
z5_E = sum_miracle(z3_E[[1]],z3_E[[2]],z4_E[[3]])
z6_E = miracle_score(z3_E[[1]], z3_E[[2]], z4_E[[3]], z5_E)
z4_T = mirna_mrna_loc(z3_T[[3]], z3_T[[4]], mirna, mrna, CWCS_matrix_cwcs)
z5_T = sum_miracle(z3_T[[1]],z3_T[[2]],z4_T[[3]])
z6_T = miracle_score(z3_T[[1]], z3_T[[2]], z4_T[[3]], z5_T)
z7 = matrix(as.numeric(z6_E[[1]]), nrow = nrow(z6_E[[1]])) #quantile normalization
z6_E[[1]] = normalize.quantiles(z7)
z8 = matrix(as.numeric(z6_T[[1]]), nrow = nrow(z6_T[[1]]))
z6_T[[1]] = normalize.quantiles(z8)
...
feature_number_miracle = matrix(data = 0, ncol = 10, nrow = times) #used to store the features selected in each CV round
feature_detail_miracle = list()
for(i in 1:times){
  feature_detail_miracle[[i]] = matrix(data = 0, ncol = 10, nrow = 1000)
}
...
for(time in 1:times){
  ...
  miracle_E = z6_E[[1]][,compare[part[[1]][[time]]]]
  miracle_T = z6_T[[1]][,compare[part[[1]][[time]]]]
  vali_miracle_E = z6_E[[1]][,compare[part[[2]][[time]]]]
  vali_miracle_T = z6_T[[1]][,compare[part[[2]][[time]]]]
  
  #the following code is used to do the cross validation and select the features from miracle predictions
  for(k in 1:10){
    temp2 = which(part_strategy[time,] == k) #for test
    temp1 = t[-temp2] #for training
    pos_miracle = feature_select_miracle(miracle_E, miracle_T, temp1) #pos_miracle is the ranked features
    feature_number_miracle[time, k] = length(pos_miracle)
    feature_detail_miracle[[time]][, k] = pos_miracle[1:1000]
  }
  #the following part is used to do the internal independent validation
  temp3 = seq(1, ncol(miracle_E), 1)
  temp4 = seq(1, ncol(vali_miracle_E), 1)
  for(j in 1:length(top_feature)){
    fea_miracle = feature_detail_miracle[[time]][1:top_feature[j], 1]
    for(k in 1:10){
      fea_miracle = intersect(fea_miracle, feature_detail_miracle[[time]][1:top_feature[j], k])
    }
    vali_sep_miracle = para_sep(miracle_E, miracle_T, temp3, fea_miracle)
    prob_E_sep_vali = test_sep(vali_sep_miracle, vali_miracle_E, temp4, fea_miracle)
    prob_T_sep_vali = test_sep(vali_sep_miracle, vali_miracle_T, temp4, fea_miracle)
    ...
    total_val_vali[j, time] = total_val_vali[j, time] + ncol(vali_miracle_E)
  }
  print(time)
}
summ_result = index_cal(top_feature, feature_detail_miracle, accu_sep_E, accu_sep_T)
feature_name = feature_trans(feature_detail_miracle, z6_E[[2]], z3_E[[3]], z3_E[[5]]) #summarize the feature information
write.table(summ_result, file = "summary result of NB_miracle.txt", quote = FALSE, sep = "\t", row.names = FALSE)
write.table(feature_name, file = "selected miracle feature.txt", quote = FALSE, sep = "\t", row.names = FALSE)
