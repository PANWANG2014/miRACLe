...
#feature_select_mirna is used to get the features for mirna expression
feature_select_mirna = function(mirna_E, mirna_T, temp){
  ...
  mean_dif = abs(mean_T - mean_E)
  pos_new = order(mean_dif, decreasing = TRUE)
  return(pos_new)
}
...
index_cal = function(top_feature, feature_detail_mirna, accu_sep_E, accu_sep_T){
  ...
  #the following code is used to calculate the stability score
  temp_inter = list()
  temp_uni = list()
  for(j in 1:length(top_feature)){
    temp_inter[[j]] =  feature_detail_mirna[[1]][1:top_feature[j],1]
    temp = feature_detail_mirna[[1]][1:top_feature[j], 1]
    for(k in 1:10){
      temp = intersect(temp, feature_detail_mirna[[time]][1:top_feature[j], k])
    }
    temp_uni[[j]] = temp
    for(time in 1:times){
      temp = feature_detail_mirna[[time]][1:top_feature[j], 1]
      for(k in 1:10){
        temp = intersect(temp, feature_detail_mirna[[time]][1:top_feature[j], k])
      }
...
feature_number_mirna = matrix(data = 0, ncol = 10, nrow = times) #used to store the features selected in each CV round
feature_detail_mirna = list()
for(i in 1:times){
  feature_detail_mirna[[i]] = matrix(data = 0, ncol = 10, nrow = 100)
}
feature_vali_number = matrix(data = 0, ncol = 10, nrow = length(top_feature)) #used to record number of features in validation step
...
for(time in 1:times){
  ...
  #the following code is used to do the cross validation and select the features from mirnas
  for(k in 1:10){
    temp2 = which(part_strategy[time,] == k) #for test
    temp1 = t[-temp2] #for training
    pos_mirna = feature_select_mirna(mirna_E, mirna_T, temp1) #pos_mirna is the ranked features
    feature_number_mirna[time, k] = length(pos_mirna)
    feature_detail_mirna[[time]][, k] = pos_mirna[1:100]
  }
  #the following code is used to do the internal independent validation
  temp3 = seq(1, ncol(mirna_E), 1)
  temp4 = seq(1, ncol(vali_mirna_E), 1)
  for(j in 1:length(top_feature)){
    fea_mirna = feature_detail_mirna[[time]][1:top_feature[j], 1]
    for(k in 1:10){
      fea_mirna = intersect(fea_mirna, feature_detail_mirna[[time]][1:top_feature[j], k])
    }
    vali_sep_mirna = para_sep(mirna_E, mirna_T, temp3, fea_mirna)
    prob_E_sep_vali = test_sep(vali_sep_mirna, vali_mirna_E, temp4, fea_mirna)
    prob_T_sep_vali = test_sep(vali_sep_mirna, vali_mirna_T, temp4, fea_mirna)
    ...
    total_val_vali[j, time] = total_val_vali[j, time] + ncol(vali_mirna_E)
  }
  print(time)
}
summ_result = index_cal(top_feature, feature_detail_mirna, accu_sep_E, accu_sep_T)

#the following code is used to get the information of the features ranked
feature_detail = feature_detail_mirna[[1]]
for(time in 2:times){
  feature_detail = cbind(feature_detail, feature_detail_mirna[[time]])
}
feature_name = matrix(data = 0, ncol = ncol(feature_detail), nrow = nrow(feature_detail))
for(i in 1:ncol(feature_name)){
  feature_name[, i] = z3_E[[3]][feature_detail[, i]]
}

write.table(summ_result, file = "summary result of NB_mirna.txt", quote = FALSE, sep = "\t", row.names = FALSE)
write.table(feature_name, file = "selected mirna ID.txt", quote = FALSE, sep = "\t", row.names = FALSE)
