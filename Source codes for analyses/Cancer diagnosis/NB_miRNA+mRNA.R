...
#ffeature_select is used to get the features
...
index_cal_mix = function(top_feature, feature_detail_mrna, feature_detail_mirna, accu_sep_E, accu_sep_T){
  summ_result = matrix(data = 0, ncol = 11, nrow = length(top_feature))
  colnames(summ_result) = c("chosen feature number", "intersect mrna number", "union mrna number",
                            "intersect mirna number", "union mirna number", "stablility score mix", "ACC", 
                            "PPV", "TPR", "TNR", "F score")
  temp_inter = list()
  temp_uni = list()
  for(j in 1:length(top_feature_sep)){
    temp_inter[[j]] = feature_detail_mrna[[1]][1:top_feature_sep[j],1]
    temp = feature_detail_mrna[[1]][1:top_feature_sep[j], 1]
    for(k in 1:10){
      temp = intersect(temp, feature_detail_mrna[[time]][1:top_feature_sep[j], k])
    }
    temp_uni[[j]] = temp
    for(time in 1:times){
      temp = feature_detail_mrna[[time]][1:top_feature_sep[j], 1]
      for(k in 1:10){
        temp = intersect(temp, feature_detail_mrna[[time]][1:top_feature_sep[j], k])
      }
  ...
  temp_inter_mirna = list()
  temp_uni_mirna = list()
  for(j in 1:length(top_feature)){
    temp_inter_mirna[[j]] = feature_detail_mirna[[1]][1:top_feature_sep[j],1]
    temp = feature_detail_mirna[[1]][1:top_feature_sep[j], 1]
    for(k in 1:10){
      temp = intersect(temp, feature_detail_mirna[[time]][1:top_feature_sep[j], k])
    }
    temp_uni_mirna[[j]] = temp
    for(time in 1:times){
      temp = feature_detail_mirna[[time]][1:top_feature_sep[j], 1]
      for(k in 1:10){
        temp = intersect(temp, feature_detail_mirna[[time]][1:top_feature_sep[j], k])
      }
      temp_inter_mirna[[j]] = intersect(temp_inter_mirna[[j]], temp)
      temp_uni_mirna[[j]] = union(temp_uni_mirna[[j]], temp)
    }
  }
  ...
  for(i in 1:length(top_feature)){
    ...
    summ_result[i, 4] = length(temp_inter_mirna[[i]])
    summ_result[i, 5] = length(temp_uni_mirna[[i]])
    summ_result[i, 6] = (length(temp_inter_mirna[[i]]) + length(temp_inter[[i]]))/(length(temp_uni_mirna[[i]]) + length(temp_uni[[i]]))
  }
  summ_result[, 1] = top_feature
  summ_result[, 7] = (rowSums(accu_sep_E) + rowSums(accu_sep_T))/(rowSums(total_val_vali) + rowSums(total_val_vali))
  summ_result[, 8] = rowSums(accu_sep_T)/(rowSums(total_val_vali) - rowSums(accu_sep_E) + rowSums(accu_sep_T))
  summ_result[, 9] = rowSums(accu_sep_T)/rowSums(total_val_vali)
  summ_result[, 10] = rowSums(accu_sep_E)/rowSums(total_val_vali)
  summ_result[, 11] = 2 * summ_result[, 9] * summ_result[, 10] / (summ_result[, 9] + summ_result[, 10])
  return(summ_result)
}
...
top_feature_sep = top_feature/2
feature_number_mrna = matrix(data = 0, ncol = 10, nrow = times) #used to store the features selected in each CV round
feature_number_mirna = matrix(data = 0, ncol = 10, nrow = times)
feature_detail_mrna = list()
feature_detail_mirna = list()
for(i in 1:times){
  feature_detail_mrna[[i]] = matrix(data = 0, ncol = 10, nrow = 100)
  feature_detail_mirna[[i]] = matrix(data = 0, ncol = 10, nrow = 100)
}
...
for(time in 1:times){
  #artition the data into training data and validation data
  ...
  mirna_E = z3_E[[1]][,compare[part[[1]][[time]]]]
  mirna_T = z3_T[[1]][,compare[part[[1]][[time]]]]
  vali_mirna_E = z3_E[[1]][,compare[part[[2]][[time]]]]
  vali_mirna_T = z3_T[[1]][,compare[part[[2]][[time]]]]
  #the following code is used to do the cross validation and select the features from mirna and mrna
  for(k in 1:10){
    ...
    pos_mirna = feature_select(mirna_E, mirna_T, temp1) #pos_mirna is the ranked features
    feature_number_mrna[time, k] = length(pos_mrna)
    feature_number_mirna[time, k] = length(pos_mirna)
    feature_detail_mrna[[time]][, k] = pos_mrna[1:50]
    feature_detail_mirna[[time]][, k] = pos_mirna[1:50]
  }
  ...
  for(j in 1:length(top_feature)){
    fea_mrna = feature_detail_mrna[[time]][1:top_feature_sep[j], 1]
    fea_mirna = feature_detail_mirna[[time]][1:top_feature_sep[j], 1]
    for(k in 1:10){
      fea_mrna = intersect(fea_mrna, feature_detail_mrna[[time]][1:top_feature_sep[j], k])
      fea_mirna = intersect(fea_mirna, feature_detail_mirna[[time]][1:top_feature_sep[j], k])
    }
    vali_sep_mrna = para_sep(mrna_E, mrna_T, temp3, fea_mrna)
    vali_sep_mirna = para_sep(mirna_E, mirna_T, temp3, fea_mirna)
    ...
    prob_E_sep_vali = prob_E_sep_vali + test_sep(vali_sep_mirna, vali_mirna_E, temp4, fea_mirna)
    prob_T_sep_vali = prob_T_sep_vali + test_sep(vali_sep_mirna, vali_mirna_T, temp4, fea_mirna)
...
summ_result = index_cal_mix(top_feature, feature_detail_mrna, feature_detail_mirna, accu_sep_E, accu_sep_T)

#the following code is used to get the information of the feature ranked
feature_detail = feature_detail_mrna[[1]]
feature_detail2 = feature_detail_mirna[[1]]
for(time in 2:times){
  feature_detail = cbind(feature_detail, feature_detail_mrna[[time]])
  feature_detail2 = cbind(feature_detail2, feature_detail_mirna[[time]])
}
...
feature_name_mirna = matrix(data = 0, ncol = ncol(feature_detail2), nrow = nrow(feature_detail2))
for(i in 1:ncol(feature_name_mirna)){
  feature_name_mirna[, i] = z3_E[[3]][feature_detail2[, i]]
}

write.table(summ_result, file = "summary result of NB_mirna+mrna.txt", quote = FALSE, sep = "\t", row.names = FALSE)
...
write.table(feature_name_mirna, file = "selected mirna ID.txt", quote = FALSE, sep = "\t", row.names = FALSE)
