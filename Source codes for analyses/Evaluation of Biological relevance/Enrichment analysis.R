...
#miracle_enrich is used to get the enrichment result of miracle
miracle_enrich = function(mirna_use1, mrna_use1, mirna_name, mrna_name, CWCS, sumup){
...
#promise_enrich is used to get the enrichment result of promise
promise_enrich = function(mirna_use1, mrna_use1, mirna_name, mrna_name, qMRE){
  mirna_use = matrix(as.numeric(mirna_use1), nrow = nrow(mirna_use1))
  mrna_use = matrix(as.numeric(mrna_use1), nrow = nrow(mrna_use1))
...
targetscan_enrich = function(CWCS, mirna_name, mrna_name){
  thres_num = floor(length(which(CWCS != 0))/100)
...
#fc_summary is used to summarize the enrichment result
fc_summary = function(z9, z10, z11, mirna_num, mirna_num_tar, z3, z4, mirna_valid){
  mirna_fc = matrix(data = 0, nrow = 100, ncol = 9, byrow = TRUE)
  colnames(mirna_fc) = c("miracle_number", "miracle_fc", "miracle_pvalue", "promise_number", "promise_fc", "promise_pvalue", 
  ...
  for(i in 1 : 100){
    mirna_fc[i, 1] = sum(z9[1:i] %in% mirna_valid)
    mirna_fc[i, 2] = (sum(z9[1:i] %in% mirna_valid)/i)/(mirna_num/length(z3))
    a1[2, 1] = mirna_fc[i, 1]
    a1[2, 2] = i - a1[2, 1]
    a1[1, 1] = mirna_num - a1[2, 1]
    a1[1, 2] = length(z3) - i - a1[1, 1]
    mirna_fc[i, 3] = fisher.test(a1, alternative = "less")$p.value
    
    mirna_fc[i, 4] = sum(z10[1:i] %in% mirna_valid)
    mirna_fc[i, 5] = (sum(z10[1:i] %in% mirna_valid)/i)/(mirna_num/length(z3))
    a2[2, 1] = mirna_fc[i, 4]
    a2[2, 2] = i - a2[2, 1]
    a2[1, 1] = mirna_num - a2[2, 1]
    a2[1, 2] = length(z3) - i - a2[1, 1]
    mirna_fc[i, 6] = fisher.test(a2, alternative = "less")$p.value
    
    mirna_fc[i, 7] = sum(z11[1:i] %in% mirna_valid)
    mirna_fc[i, 8] = (sum(z11[1:i] %in% mirna_valid)/i)/(mirna_num_tar/length(z4))
    a3[2, 1] = mirna_fc[i, 7]
    a3[2, 2] = i - a3[2, 1]
    a3[1, 1] = mirna_num_tar - a3[2, 1]
    a3[1, 2] = length(z4) - i - a3[1, 1]
    mirna_fc[i, 9] = fisher.test(a3, alternative = "less")$p.value
  }
  return(mirna_fc)
}
...
#input of known cancer-related elements: Oncomir set, miRNA biomarker set and Cancer gene set
...
mirna_valid1 = as.matrix(read.table("Oncomir set.txt", head = TRUE, sep = "\t"))
mirna_valid2 = as.matrix(read.table("miRNA biomarker set.txt", head = TRUE, sep = "\t"))
mrna_valid = as.matrix(read.table("Cancer gene set.txt", head = TRUE, sep = "\t"))
...
#the following code is used to get the predicted mirnas or mrnas in order
z8 = sum_miracle(z3[[1]], z3[[2]], z7[[3]])
z9 = miracle_enrich(z3[[1]], z3[[2]], z3[[3]], z3[[4]], z7[[3]], z8)
z10 = promise_enrich(z3[[1]], z3[[2]], z3[[3]], z3[[4]], z6[[3]])
z11 = targetscan_enrich(z7_tar[[3]], z4[[3]], z4[[4]])

#mirna_num and mrna_num is the overlap of known cancer-related miRNAs and mRNAs in the matrix
...
#the following code is used to get the final output
mirna_fc1 = fc_summary(z9[[1]], z10[[1]], z11[[1]], mirna_num1, mirna_num1_tar, z3[[3]], z4[[3]], mirna_valid1)
mirna_fc2 = fc_summary(z9[[1]], z10[[1]], z11[[1]], mirna_num2, mirna_num2_tar, z3[[3]], z4[[3]], mirna_valid2)
mrna_fc = fc_summary(z9[[2]], z10[[2]], z11[[2]], mrna_num, mrna_num_tar, z3[[4]], z4[[4]], mrna_valid[, 1])
write.table(mirna_fc1,file = "Enrichment of oncomirs.txt", quote = FALSE, sep = "\t")
write.table(mirna_fc2,file = "Enrichment of miRNA biomarkers.txt", quote = FALSE, sep = "\t")
write.table(mrna_fc,file = "Enrichment of cancer genes.txt", quote = FALSE, sep = "\t")